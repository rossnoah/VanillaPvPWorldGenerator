...
public class SnowChunkGenerator extends ChunkGenerator {
    ...
    public void generateNoise(WorldInfo worldInfo, Random random, int chunkX, int chunkZ, ChunkData chunkData) {
        ...
        // 3) For each column, place stone / snow block
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                int snowBottom = stoneTop;
                int snowTop    = snowBottom + 5;
                if (snowTop > maxY) {
                    snowTop = maxY;
                }
                if (snowTop > snowBottom) {
                    chunkData.setRegion(x, snowBottom, z, x + 1, snowTop, z + 1, Material.SNOW_BLOCK);
                }

                int surfaceHeight = snowTop - 1;
                if (surfaceHeight < maxY - 1 && surfaceHeight >= chunkData.getMinHeight()) {
                    ...
                    if (detailValue > 0.9 && chunkRandom.nextDouble() < 0.1) {
                        chunkData.setBlock(x, surfaceHeight + 1, z, Material.SPRUCE_LOG);
                    }
                }
            }
        }
